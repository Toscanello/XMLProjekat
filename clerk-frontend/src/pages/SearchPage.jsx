...
function SearchPage(){
    ...
    function handleClick(e){
        ...
        getAllDocumentsForPhrase(searchField, (response) => {
            ...
            parseXmlToJs(response.data.replaceAll('ns2:', '').replaceAll('ns3:', ''), (result) => {
                ...
                if(result.allDocuments.digitalCertificates[0]){
                    setCertificates(result.allDocuments.digitalCertificates[0].certificate);
                }
                if(result.allDocuments.vaccinationReports[0]){
                    setReports(result.allDocuments.vaccinationReports[0].vaccinationReport);
                }
            })
        });
    }
        ...
        <>
            ...
            <h2>Saglasnosti</h2>
            {
                accordances.map(accordance => {
                    return <div><h4>{accordance.jmbg}</h4>
                                <button id={"accordances/downloadhtml/" +accordance.jmbg+"_"+accordance.date} onClick={displayFile}>Prikazi saglasnost</button></div>
                })
            }
            <h2>Sertifikati</h2>
            {
                certificates.map(cert => {
                    return <div><h4>{cert.id}</h4>
                                <button id={"certificates/downloadhtml/"+cert.id} onClick={displayFile}>Prikazi sertifikat</button></div>
                })
            }
            <h2>Izvestaji o vakcinaciji</h2>
            {
                reports.map(report => {
                    return <div>
                               <h4>{count++}</h4>
                                <button id={"vaccine-reports/downloadhtml/"+count} onClick={displayFile}>Prikazi izvestaj</button>
                           </div>
                })
            }
        </>
